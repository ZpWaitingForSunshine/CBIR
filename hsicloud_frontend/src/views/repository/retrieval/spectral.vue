  ...
  <el-row>
    <el-col :span="8">
      <div class="grid-content bg-purple left-content">
        ...
        <template v-for="(prop, index) in items">
          <div slot="header" @click.stop="getSpec(prop.id,index)" :key="prop.id" class="">
              <br>
            <li class="infinite-list-item">{{ prop.name }}</li>
          </div>
          <dd v-for="op in prop.options" :key="op.id">
            <input v-model="op.status" type="checkbox" :value="op" @change="showSpec(op)">{{ " " + op.value }}
          </dd>
        </template>
      </div>
    </el-col>
      ...
      <div>
        <div>
          ...
          <el-form ref="formabundance" label-width="30%" >
            <el-form-item v-for="op in allImgDate" :label="op.specname" :key="op.id">
    ...
    <el-col :span="8">
        <div>
          ...
          <el-table :data="result" border fit highlight-current-row style="width: 100%">
...
import {fetchSpectralNames, fetchSpectralById, fetchSpectralInformationByID} from '@/api/spectrallibrary'
  ...
  methods: {
    ...
    getLib() {
      fetchSpectralNames().then(response => {
        response.data.forEach((x, index) => {
          this.items.push({
            'name': x.name, 'type': 1, 'id':x.id, 'options': [
            ]
          })
        })
    ...
    showSpec(op) { // 加载光谱图片
      ...
      if (op.status == true) {
        ...
        console.log('quexiao')
        this.allImgDate.splice(this.allImgDate.findIndex(item => item.id === op.id), 1)
      }
    },
    search_now(v1, v2) { // 搜索功能
      console.log(v1 + v2)
    },
    JumpToList(id) {
    ...
    onSubmit(){
      ...
      searchImage(this.abundanceForm).then(
        res => {
